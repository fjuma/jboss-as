...
public class IdentityResourceDefinition extends SimpleResourceDefinition {
    ...
    private static boolean isOutflowSecurityDomainsConfigured;
    ...
    private static class AddHandler extends AbstractAddStepHandler {
        ...
        protected void populateModel(OperationContext context, ModelNode operation, Resource resource) throws OperationFailedException {
            ...
            isOutflowSecurityDomainsConfigured = ! (OUTFLOW_SECURITY_DOMAINS.unwrap(context, resource.getModel())).isEmpty();
        }

        @Override
        protected void performRuntime(OperationContext context, ModelNode operation, ModelNode model) throws OperationFailedException {
            IdentityService identityService = new IdentityService();
            ServiceBuilder<Consumer<SecurityIdentity>> serviceBuilder = context.getServiceTarget()
                    .addService(IDENTITY_RUNTIME_CAPABILITY.getCapabilityServiceName(), identityService)
                    .setInitialMode(Mode.LAZY);
            List<String> outflowSecurityDomains = OUTFLOW_SECURITY_DOMAINS.unwrap(context, model);
    ...
    static class IdentityService implements Service<Consumer<SecurityIdentity>> {

        private final List<InjectedValue<SecurityDomain>> outflowSecurityDomainInjectors = new ArrayList<>();
        private List<SecurityDomain> outflowSecurityDomains = new ArrayList<>();
        ...
        private void outflowIdentity(final SecurityIdentity securityIdentity) {
            if ((securityIdentity != null) && (outflowSecurityDomains != null)) {
                ...
                for (SecurityDomain outflowSecurityDomain : outflowSecurityDomains) {
                    ...
                    try {
                        serverAuthenticationContext.importIdentity(securityIdentity);
                    } catch (RealmUnavailableException e) {
                        // Ignored
                    }
                }
                // FJ, maybe need to pass in interceptor context, do interceptor context.proceed here, and then finally set back to old domains,
                // would need to use ***list*** instead of set, biconsumer
            }
        }
        ...
        @Override
        public Consumer<SecurityIdentity> getValue() throws IllegalStateException, IllegalArgumentException {
    ...
    boolean isOutflowSecurityDomainsConfigured() {
        return isOutflowSecurityDomainsConfigured;
    }

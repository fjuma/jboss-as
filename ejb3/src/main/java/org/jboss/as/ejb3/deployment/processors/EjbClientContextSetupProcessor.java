...
public class EjbClientContextSetupProcessor implements DeploymentUnitProcessor {
    ...
    private static final class RegistrationService implements Service<Void> {
        ...
        private static AuthenticationContext transformOne(RemotingProfileService.ConnectionSpec connectionSpec, AuthenticationContext context) throws StartException {
            ...
            if (sslContext == null) {
                try {
                    sslContext = CLIENT.getSSLContext(destinationUri, context, null, null, null);
                } catch (GeneralSecurityException e) {
                    throw EjbLogger.ROOT_LOGGER.failedToObtainSSLContext(e);
                }
            }
            final SSLContext finalSSLContext = sslContext;
            return context.with(0, rule, configuration.with(authenticationConfiguration)).withSsl(0, rule, () -> finalSSLContext);
        }
    }
}

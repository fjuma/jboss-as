...
/**
 * A {@link Service} to return a mapping of security domains by name.
 *
...
public class SecurityDomainsService implements Service<Map<String, SecurityDomain>> {
    ...
    public void start(final StartContext context) throws StartException {
        securityDomainsByName = new HashMap<String, SecurityDomain>(injectedSecurityDomainsByName.size());
        injectedSecurityDomainsByName.forEach((key, value) -> securityDomainsByName.put(key, value.getValue()));
        // FJ
        for (String name : securityDomainsByName.keySet()) {
            System.out.println("*** MAP SECURITY DOMAIN NAME " + name);
            System.out.println("*** MAP SECURITY DOMAIN IS " + securityDomainsByName.get(name).toString());
        }
    }
    ...
    public Map<String, SecurityDomain> getValue() throws IllegalStateException, IllegalArgumentException {
        return Collections.unmodifiableMap(securityDomainsByName);
    }

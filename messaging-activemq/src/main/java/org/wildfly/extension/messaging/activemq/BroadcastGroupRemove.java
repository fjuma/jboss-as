...
public class BroadcastGroupRemove extends AbstractRemoveStepHandler {
    ...
    public void execute(OperationContext context, ModelNode operation) throws OperationFailedException {
        if (!isLegacyCall) {
            ...
            try {
                context.readResourceFromRoot(target.append(CommonAttributes.JGROUP_BROADCAST_GROUP, context.getCurrentAddressValue()));
                target = target.append(CommonAttributes.JGROUP_BROADCAST_GROUP, context.getCurrentAddressValue());
                removeHandler = JGroupBroadcastGroupRemove.LEGACY_INSTANCE;

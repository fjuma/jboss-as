...
import static org.wildfly.extension.messaging.activemq.CommonAttributes.JGROUP_BROADCAST_GROUP;
import static org.wildfly.extension.messaging.activemq.CommonAttributes.JGROUP_DISCOVERY_GROUP;
        ...
        public void execute(OperationContext context, ModelNode operation) throws OperationFailedException {
            ...
            if (broadcastGroupConfigurations != null) {
                for (final BroadcastGroupConfiguration config : broadcastGroupConfigurations) {
                    ...
                    if (model.hasDefined(JGROUP_BROADCAST_GROUP, name)) {
                        ModelNode broadcastGroupModel = model.get(JGROUP_BROADCAST_GROUP, name);
            ...
            if (discoveryGroupConfigurations != null) {
                for (final DiscoveryGroupConfiguration config : discoveryGroupConfigurations.values()) {
                    ...
                    if (model.hasDefined(JGROUP_DISCOVERY_GROUP, name)) {
                        ModelNode discoveryGroupModel = model.get(JGROUP_DISCOVERY_GROUP, name);

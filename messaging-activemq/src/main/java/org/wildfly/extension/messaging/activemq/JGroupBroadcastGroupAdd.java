 ...
 */
public class JGroupBroadcastGroupAdd extends AbstractAddStepHandler {

    public static final JGroupBroadcastGroupAdd INSTANCE = new JGroupBroadcastGroupAdd(true);
    public static final JGroupBroadcastGroupAdd LEGACY_INSTANCE = new JGroupBroadcastGroupAdd(false);

    private final boolean needLegacyCall;
    private JGroupBroadcastGroupAdd(boolean needLegacyCall) {
        super(JGroupBroadcastGroupDefinition.ATTRIBUTES);
    ...
    static void addBroadcastGroupConfigs(final OperationContext context, final List<BroadcastGroupConfiguration> configs, final Set<String> connectors, final ModelNode model)  throws OperationFailedException {
        if (model.hasDefined(CommonAttributes.JGROUP_BROADCAST_GROUP)) {
            for (Property prop : model.get(CommonAttributes.JGROUP_BROADCAST_GROUP).asPropertyList()) {

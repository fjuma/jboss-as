...
import static org.wildfly.extension.messaging.activemq.JGroupDiscoveryGroupDefinition.JGROUPS_CHANNEL;
import static org.wildfly.extension.messaging.activemq.JGroupDiscoveryGroupDefinition.JGROUPS_CHANNEL_FACTORY;
 ...
 */
public class JGroupDiscoveryGroupAdd extends AbstractAddStepHandler {

    public static final JGroupDiscoveryGroupAdd INSTANCE = new JGroupDiscoveryGroupAdd(true);
    public static final JGroupDiscoveryGroupAdd LEGACY_INSTANCE = new JGroupDiscoveryGroupAdd(false);

    private final boolean needLegacyCall;

    private JGroupDiscoveryGroupAdd(boolean needLegacyCall) {
        super(JGroupDiscoveryGroupDefinition.ATTRIBUTES);
    ...
    static Map<String, DiscoveryGroupConfiguration> addDiscoveryGroupConfigs(final OperationContext context, final ModelNode model)  throws OperationFailedException {
        ...
        if (model.hasDefined(CommonAttributes.JGROUP_DISCOVERY_GROUP)) {
            for (Property prop : model.get(CommonAttributes.JGROUP_DISCOVERY_GROUP).asPropertyList()) {

...
public class BroadcastGroupAdd extends ShallowResourceAdd {
    ...
    private BroadcastGroupAdd(boolean isLegacyCall) {
        super(isLegacyCall, BroadcastGroupDefinition.ATTRIBUTES);
    }
       ...
       CommonAttributes.renameChannelToCluster(operation);
        if (!isLegacyCall) {
            ...
            if (operation.hasDefined(JGROUPS_CLUSTER.getName())) {
                target = target.append(CommonAttributes.JGROUP_BROADCAST_GROUP, context.getCurrentAddressValue());
                addHandler = JGroupBroadcastGroupAdd.LEGACY_INSTANCE;
            } else {
